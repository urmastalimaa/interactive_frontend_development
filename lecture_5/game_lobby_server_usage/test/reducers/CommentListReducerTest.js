...
describe('CommentListReducer', () => {
  ...
  it('adds in-flight comment when comment post requested', () => {
    ...
    expect(
      reducer(undefined, commentPostRequested({...comment, localId: 'localId'}))
    ).to.eql({
  ...
  it('sets comment as not in flight when post succeeds', () => {
    ...
    const initialState =
      reducer(undefined, commentPostRequested({...comment, localId: 'localId'}));
    expect(
      reducer(initialState, commentPostSucceeded({id: 'id', localId: 'localId'}))
    ).to.eql({
  ...
  it('removes comment when post fails', () => {
    ...
    const initialState =
      reducer(undefined, commentPostRequested({...comment, localId: 'localId'}));
    expect(
      reducer(initialState, commentPostFailed({localId: 'localId', error: 'error'}))
    ).to.eql({
      comments: [],
      fetchState: {inFlight: false}
    });
